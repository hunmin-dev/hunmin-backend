...
class AuthServiceTest : BehaviorSpec({
    ...
    val authService = AuthService(authRepositoryPort, authPasswordEncryptor, tokenProviderPort, eventPublisher)
    ...
    Given("회원 가입을 할 때") {
        ...
        When("닉네임이 존재하지 않으면") {
            ...
            // publishEvent 모킹 추가
            every { eventPublisher.publishEvent(any<AuthCreatedEvent>()) } returns Unit

            val command = 인증_생성_커맨드()
            Then("정상 가입이 되고 토큰을 반환한다") {
                val response = authService.signUp(command)
                response shouldBe "token"
                verify { eventPublisher.publishEvent(any<AuthCreatedEvent>()) }
            }
        }
    }
