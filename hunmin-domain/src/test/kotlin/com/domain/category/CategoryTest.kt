...
class CategoryTest : BehaviorSpec({
    ...
    Given("Category를 생성할 때") {
        ...
        When("값이 주어지지 않으면") {
            Then("id와 createdAt은 0이고, isVisible은 true 이다") {
        ...
        When("값이 명시적으로 주어지면") {
            Then("해당 값으로 생성된다") {
                val category = Category(id = 1L, title = "initial-title", isVisible = false, createdAt = 1000L)
    ...
    Given("기존 Category가 존재할 때") {
        val category = Category(id = 1L, title = "initial-title", isVisible = true, createdAt = 1000L)
